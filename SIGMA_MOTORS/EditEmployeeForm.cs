        ...
        private void LoadEmployee()
        {
            try
            {
                using (var conn = new MySqlConnection(ConnStr))
                {
                    conn.Open();
                    const string sql =
                        "SELECT NAME, CNIC, ROLE, LOGIN_ID, PASSWORD, IS_ACTIVE " +
                    ...
                    using (var cmd = new MySqlCommand(sql, conn))
                    {
                        ...
                        using (var rdr = cmd.ExecuteReader())
                        {
                            if (!rdr.Read())
                            {
                                MessageBox.Show("Employee record not found.", "Error",
                            ...
                            txtRole.ForeColor = string.IsNullOrWhiteSpace(txtRole.Text)
                                                ? Color.Gray : Color.Black;
                            if (txtRole.ForeColor == Color.Gray)
                                txtRole.Text = "Role";

                            txtLoginId.Text = rdr["LOGIN_ID"].ToString();
                            ...
                            chkIsActive.Checked = Convert.ToBoolean(rdr["IS_ACTIVE"]);
                        }
                    }
                }
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (var conn = new MySqlConnection(ConnStr))
                {
                    conn.Open();
                    const string sql =
                        "UPDATE EMPLOYEE SET NAME=@name, CNIC=@cnic, ROLE=@role, " +
                        "LOGIN_ID=@loginId, PASSWORD=@password, IS_ACTIVE=@isActive " +
